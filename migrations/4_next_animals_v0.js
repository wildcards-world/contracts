  ...
  deployer.then(async () => {
    ...
    await Promise.all(
      tokenData.map(token => {
        ...
        return patronageToken.mintWithTokenURI(steward.address, token.id, JSON.stringify(token.metadata), { from: accounts[0] })
      })
    )
    ...
    await steward.listNewTokens(listOfTokenIds, benefactors, patronageNumerators, { from: accounts[0] })
  })
}
