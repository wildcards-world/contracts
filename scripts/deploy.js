...
const { AppProject, Contracts, ZWeb3, Package } = require('@openzeppelin/upgrades')
...
async function deployVersion1(project, owner) {
  ...
  const WildcardSteward = await project.createProxy(WildcardSteward_v0, { contractName: wildcardStewardName })
  ...
  const ERC721Patronage = await project.createProxy(
    ERC721Patronage_v0,
    {
  ...
  await ERC721Patronage.methods.setup(
    WildcardSteward.address, "WildcardsTokens", "WT", owner
  ).send({ from: owner })
  await ERC721Patronage.methods.mintWithTokenURI(steward.address, 0, image1MetadataString).send({ from: owner })
  await ERC721Patronage.methods.mintWithTokenURI(steward.address, 1, image2MetadataString).send()
  await WildcardSteward.methods.initialize([0, 1], accounts[0], patronageToken.address, patronageNumerator, patronageDenominator)
