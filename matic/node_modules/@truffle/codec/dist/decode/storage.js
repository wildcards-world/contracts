...
const debug = debug_1.default("codec:decode:storage");
...
function* decodeStorageReferenceByAddress(dataType, pointer, info) {
    ...
    let rawSize;
    try {
        rawSize = storage_1.storageSizeForType(dataType, info.userDefinedTypes, allocations);
    }
...
function* decodeStorageReference(dataType, pointer, info) {
    ...
    switch (dataType.typeClass) {
        case "array": {
            ...
            let baseSize;
            try {
                baseSize = storage_1.storageSizeForType(dataType.baseType, info.userDefinedTypes, allocations);
            }
            catch (error) { //error: DecodingError
                return {
                    type: dataType,
            ...
            let ranges = [];
            if (storage_2.isWordsLength(baseSize)) {
        ...
        case "string": {
            ...
            if (lengthByte % 2 == 0) {
                ...
                return yield* value_1.default(dataType, { location: "storage", range: {
                        from: { slot: pointer.range.from.slot, index: 0 },
                        to: { slot: pointer.range.from.slot, index: length - 1 }
                    } }, info);
            }
            else {
                let lengthAsBN = CodecUtils.Conversion.toBN(data).subn(1).divn(2);
                try {
                    length = lengthAsBN.toNumber();
                }
                catch (_) {
                ...
                return yield* value_1.default(dataType, { location: "storage",
                    range: {
                        from: {
                            slot: {
                                path: pointer.range.from.slot,
        ...
        case "struct": {
            ...
            for (let index = 0; index < members.length; index++) {
                ...
                let memberType = utils_1.TypeUtils.specifyLocation(storedMemberType, "storage");
                decodedMembers.push({
                    name: memberName,
        ...
        case "mapping": {
            ...
            let valueSize;
            try {
                valueSize = storage_1.storageSizeForType(valueType, info.userDefinedTypes, allocations);
            }
            catch (error) { //error: DecodingError
                return {
                    type: dataType,
            ...
            for (const { key } of keySlots) {
                let valuePointer;
                if (storage_2.isWordsLength(valueSize)) {
                    valuePointer = {
                        location: "storage",
                        range: {
                            from: {
