...
class RelayServer extends events_1.EventEmitter {
    constructor(params) {
        ...
        this.minAlertedDelayMS = 5000;
        this.maxAlertedDelayMS = 40000;
        this.nonces = {};
        this.gasPrice = 0;
        ...
        this.maxAcceptanceBudget = (_e = params.maxAcceptanceBudget) !== null && _e !== void 0 ? _e : defaultMaxAcceptanceBudget;
        this.url = (_f = params.url) !== null && _f !== void 0 ? _f : 'http://localhost:8090';
        this.workerMinBalance = (_g = params.workerMinBalance) !== null && _g !== void 0 ? _g : defaultWorkerMinBalance;
        this.workerTargetBalance = (_h = params.workerTargetBalance) !== null && _h !== void 0 ? _h : defaultWorkerTargetBalance;
        this.managerMinBalance = (_j = params.managerMinBalance) !== null && _j !== void 0 ? _j : defaultManagerMinBalance;
        this.managerTargetBalance = (_k = params.managerTargetBalance) !== null && _k !== void 0 ? _k : defaultManagerTargetBalance;
        this.minHubWithdrawalBalance = (_l = params.minHubWithdrawalBalance) !== null && _l !== void 0 ? _l : defaultMinHubWithdrawalBalance;
        this.alertedBlockDelay = (_m = params.alertedBlockDelay) !== null && _m !== void 0 ? _m : defaultAlertedBlockDelay;
    ...
    async createRelayTransaction(req) {
        ...
        if (!viewRelayCallRet.paymasterAccepted) {
            throw new Error(`Paymaster rejected in server: ${Utils_1.decodeRevertReason(viewRelayCallRet.returnValue)} req=${JSON.stringify(relayRequest, null, 2)}`);
        }
        // Send relayed transaction
        debug('maxPossibleGas is', typeof maxPossibleGas, maxPossibleGas);
        const maxCharge = await this.relayHubContract.calculateCharge(maxPossibleGas, {
        ...
        if (this.alerted) {
            ...
            await Utils_1.sleep(Utils_1.randomInRange(this.minAlertedDelayMS, this.maxAlertedDelayMS));
        }
        return signedTx;
    }
    start() {
    ...
    async _init() {
        ...
        this.smTopics = stakeManagerTopics.concat([[Utils_1.address2topic(this.managerAddress)]]);
        const relayHubTopics = [Object.keys(this.relayHubContract.contract.events).filter(x => (x.includes('0x')))];
        this.rhTopics = relayHubTopics.concat([[Utils_1.address2topic(this.managerAddress)]]);
    ...
    async replenishServer(workerIndex) {
        var _a, _b, _c, _d;
        const receipts = [];
        ...
        if (managerEthBalance.gte(web3_utils_1.toBN(this.managerTargetBalance.toString())) && workerBalance.gte(web3_utils_1.toBN(this.workerMinBalance.toString()))) {
            // all filled, nothing to do
            return receipts;
        }
        if (managerEthBalance.lt(web3_utils_1.toBN(this.managerTargetBalance.toString())) && managerHubBalance.gte(web3_utils_1.toBN(this.minHubWithdrawalBalance))) {
            ...
            const method = (_c = this.relayHubContract) === null || _c === void 0 ? void 0 : _c.contract.methods.withdraw(web3_utils_1.toHex(managerHubBalance), this.getManagerAddress());
        ...
        if (workerBalance.lt(web3_utils_1.toBN(this.workerMinBalance.toString()))) {
            const refill = web3_utils_1.toBN(this.workerTargetBalance.toString()).sub(workerBalance);
            console.log(`== replenishServer: mgr balance=${managerEthBalance.toString()}  manager hub balance=${managerHubBalance.toString()} 
          worker balance=${workerBalance.toString()} refill=${refill.toString()}`);
            if (refill.lt(managerEthBalance.sub(web3_utils_1.toBN(this.managerMinBalance)))) {
    ...
    async _worker(blockHeader) {
        ...
        if (balance.lt(web3_utils_1.toBN(this.managerMinBalance))) {
            throw new StateError(`Server's balance too low ( ${balance.toString()}, required ${this.managerMinBalance}). Waiting for funding...`);
        }
    ...
    async _getContractLogs(address, topics) {
        const options = {
    ...
    async _handlePastEvents(blockHeader) {
        var _a, _b;
        const smLogs = await this._getContractLogs((_a = this.stakeManagerContract) === null || _a === void 0 ? void 0 : _a.address, this.smTopics);
        const rhLogs = await this._getContractLogs((_b = this.relayHubContract) === null || _b === void 0 ? void 0 : _b.address, this.rhTopics);
        spam('logs?', smLogs, rhLogs);
        let decodedLogs = abi_decoder_1.default.decodeLogs(smLogs).map(this._parseEvent);
        decodedLogs = decodedLogs.concat(abi_decoder_1.default.decodeLogs(rhLogs).map(this._parseEvent));
    ...
    async _getRegistrationBlock() {
        ...
        const event = relayRegisteredEvents.find((e) => e.returnValues.relayManager.toLowerCase() === this.managerAddress.toLowerCase() &&
    ...
    async _getLatestTxBlockNumber() {
        const events = await this.contractInteractor.getPastEventsForHub(Constants_1.constants.activeManagerEvents, [Utils_1.address2topic(this.managerAddress)], {
            fromBlock: 1
        });
        const latestBlock = events.filter((e) => /* e.returnValues.relayManager != null && */ e.returnValues.relayManager.toLowerCase() === this.managerAddress.toLowerCase()).map((e) => e.blockNumber).reduce((b1, b2) => Math.max(b1, b2));
        return latestBlock;
    }
    ...
    async _handleTransactionRejectedByPaymasterEvent(dlog, blockNumber) {
