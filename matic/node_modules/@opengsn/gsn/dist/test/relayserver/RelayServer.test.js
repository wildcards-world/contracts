...
// @ts-ignore
abi_decoder_1.default.addABI(TestPaymasterConfigurableMisbehavior.abi);
...
contract.only('RelayServer', function (accounts) {
    ...
    async function bringUpNewRelay(overrideParams) {
        ...
        const interactor = new ContractInteractor_1.default(serverWeb3provider, GSNConfigurator_1.configureGSN({ relayHubAddress: rhub.address }));
        const params = Object.assign({ txStoreManager,
            managerKeyManager,
            workersKeyManager, hubAddress: rhub.address, url: localhostOne, baseRelayFee: 0, pctRelayFee: 0, gasPriceFactor: 1, contractInteractor: interactor, devMode: true }, overrideParams);
    ...
    async function assertTransactionRelayed(server, txhash, gasLess) {
        ...
        const event1 = decodedLogs.find((e) => e.name === 'SampleRecipientEmitted');
        assert.equal(event1.args.message, 'hello world');
        const event2 = decodedLogs.find((e) => e.name === 'TransactionRelayed');
    ...
    async function relayTransaction(server, options, overrideArgs, assertRelayed = true) {
        ...
        return await relayTransactionFromRequest(server, overrideArgs !== null && overrideArgs !== void 0 ? overrideArgs : {}, { relayRequest, relayMaxNonce, approvalData, signature, httpRequest }, assertRelayed);
    }
    async function relayTransactionFromRequest(server, overrideArgs, { relayRequest, relayMaxNonce, approvalData, signature, httpRequest }, assertRelayed = true) {
    ...
    describe('relay workers/manager rebalancing', function () {
        ...
        const txcost = web3_utils_1.toBN(Environments_1.defaultEnvironment.mintxgascost * parseInt(gasPrice));
        ...
        it('should withdraw hub balance to manager first, then use eth balance to fund workers', async function () {
            ...
            await _web3.eth.sendTransaction({ from: accounts[0], to: relayServer.getManagerAddress(), value: relayServer.managerTargetBalance - 1e7 });
            ...
            const workerBalanceBefore = await relayServer.getWorkerBalance(workerIndex);
            const refill = web3_utils_1.toBN(relayServer.workerTargetBalance.toString()).sub(workerBalanceBefore);
            assert.isTrue(managerHubBalanceBefore.gte(refill), 'manager hub balance should be sufficient to replenish worker');
            assert.isTrue(managerEthBalanceBefore.lt(web3_utils_1.toBN(relayServer.managerTargetBalance.toString())), 'manager eth balance should be lower than target to withdraw hub balance');
            const receipts = await relayServer.replenishServer(workerIndex);
            ...
            assert.isTrue(managerEthBalanceAfter.eq(managerEthBalanceBefore.add(managerHubBalanceBefore).sub(refill).sub(totalTxCosts)), 'manager eth balance should increase by hub balance minus txs costs');
        });
        ...
        describe('_registerIfNeeded', function () {
            ...
            describe('with re-registration', function () {
                ...
                it('re-register server when registrationBlockRate passed from any tx', async function () {
                    let receipts = await newServer._registerIfNeeded();
                    assertRelayAdded(receipts, newServer);
                    await TestUtils_1.evmMineMany(registrationBlockRate);
                    receipts = await newServer._registerIfNeeded();
                    assertRelayAdded(receipts, newServer, false);
                });
                it('do not re-register server before registrationBlockRate passed from any tx', async function () {
                    let receipts = await newServer._registerIfNeeded();
                    assertRelayAdded(receipts, newServer);
                    await TestUtils_1.evmMineMany(registrationBlockRate - 1);
                    receipts = await newServer._registerIfNeeded();
                    assert.equal(receipts.length, 0, 'should not re-register if already registered');
                });
                it('do not re-register server when registrationBlockRate passed from registration but not from relayed tx', async function () {
                    let receipts = await newServer._registerIfNeeded();
                    assertRelayAdded(receipts, newServer);
                    await newServer._worker(await _web3.eth.getBlock('latest'));
                    await TestUtils_1.evmMineMany(registrationBlockRate - 10);
                    await relayTransaction(newServer, options);
                    await TestUtils_1.evmMineMany(registrationBlockRate - 20);
                    receipts = await newServer._registerIfNeeded();
    ...
    describe('alerted state as griefing mitigation', function () {
        ...
        async function attackTheServer(server) {
            const _sendTransactionOrig = server._sendTransaction;
            const _sendTransaction = async function ({ signer, method, destination, value = '0x', gasLimit, gasPrice }) {
        ...
        it('should exit alerted state after the configured blocks delay', async function () {
            await TestUtils_1.evmMineMany(newServer.alertedBlockDelay - 1);
            await newServer._worker(await _web3.eth.getBlock('latest'));
            assert.isTrue(newServer.alerted, 'server not alerted');
            await TestUtils_1.evmMineMany(2);
            await newServer._worker(await _web3.eth.getBlock('latest'));
