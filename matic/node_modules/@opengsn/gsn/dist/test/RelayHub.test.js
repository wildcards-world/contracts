    ...
    describe('relayCall', function () {
        ...
        context('with unknown worker', function () {
            ...
            it('should not accept a relay call', async function () {
                await test_helpers_1.expectRevert(relayHubInstance.relayCall(10e6, relayRequest, signature, approvalData, gas, {
                    from: relayWorker,
                    gas
        ...
        context('with staked and registered relay', function () {
            ...
            context('with relay worker that is not externally-owned account', function () {
                it('should not accept relay requests', async function () {
                    ...
                    await test_helpers_1.expectRevert(testRelayWorkerContract.relayCall(relayHubInstance.address, 10e6, relayRequest, signature, gas, {
                        gas
            ...
            context('with view functions only', function () {
                ...
                it('should get \'paymasterAccepted = true\' and no revert reason as view call result of \'relayCall\' for a valid transaction', async function () {
                    const relayCallView = await relayHubInstance.contract.methods.relayCall(10e6, relayRequest, signatureWithPermissivePaymaster, '0x', 7e6)
                        .call({
                        from: relayWorker,
                        gas: 7e6
                    });
                ...
                it('should get Paymaster\'s reject reason from view call result of \'relayCall\' for a transaction with a wrong signature', async function () {
                    ...
                    assert.equal(relayCallView.returnValue, TestUtils_1.encodeRevertReason('invalid code'));
                    assert.equal(Utils_1.decodeRevertReason(relayCallView.returnValue), 'invalid code');
                });
            });
            context('with funded paymaster', function () {
                ...
                it('relayCall executes the transaction and increments sender nonce on hub', async function () {
                    ...
                    test_helpers_1.expectEvent.inLogs(logs, 'TransactionResult', {
                ...
                it('relayCall should refuse to re-send transaction with same nonce', async function () {
                    ...
                    await test_helpers_1.expectEvent(ret, 'TransactionRejectedByPaymaster', { reason: TestUtils_1.encodeRevertReason('nonce mismatch') });
                });
                ...
                it('postRelayedCall receives values returned in preRelayedCall', async function () {
                    const { tx } = await relayHubInstance.relayCall(10e6, relayRequestPaymasterWithContext, signatureWithContextPaymaster, '0x', gas, {
                        from: relayWorker,
                        gas,
                        gasPrice
                    });
                ...
                it('relaying is aborted if the paymaster reverts the preRelayedCall', async function () {
                    await misbehavingPaymaster.setReturnInvalidErrorCode(true);
                    const { logs } = await relayHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster, signatureWithMisbehavingPaymaster, '0x', gas, {
                        from: relayWorker,
                        gas,
                        gasPrice
                    });
                    test_helpers_1.expectEvent.inLogs(logs, 'TransactionRejectedByPaymaster', { reason: TestUtils_1.encodeRevertReason('invalid code') });
                });
                it('should not accept relay requests if gas limit is too low for a relayed transaction', async function () {
                    ...
                    await test_helpers_1.expectRevert(relayHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster, signatureWithMisbehavingPaymaster, '0x', gas, {
                        from: relayWorker,
                        gasPrice,
                        gas
                ...
                it('should not accept relay requests with gas limit higher then block gas limit', async function () {
                    await test_helpers_1.expectRevert(relayHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster, signatureWithMisbehavingPaymaster, '0x', 100000001, {
                        from: relayWorker,
                        gasPrice,
                        gas
                ...
                it('should not accept relay requests if destination recipient doesn\'t have a balance to pay for it', async function () {
                    ...
                    await test_helpers_1.expectRevert(relayHubInstance.relayCall(10e6, relayRequestPaymaster2, signatureWithMisbehavingPaymaster, '0x', gas, {
                        from: relayWorker,
                        gas,
                        gasPrice
                ...
                it('should not execute the \'relayedCall\' if \'preRelayedCall\' reverts', async function () {
                    ...
                    test_helpers_1.expectEvent.inLogs(logs, 'TransactionRejectedByPaymaster', {
                        reason: TestUtils_1.encodeRevertReason('You asked me to revert, remember?')
                    });
                });
                ...
                describe('recipient balance withdrawal ban', function () {
                    ...
                    async function assertRevertWithPaymasterBalanceChanged() {
                        const { logs } = await relayHubInstance.relayCall(10e6, relayRequestMisbehavingPaymaster, signature, '0x', gas, {
                            from: relayWorker,
                            gas,
                            gasPrice
                        });
                        test_helpers_1.expectEvent.inLogs(logs, 'TransactionRelayed', { status: RelayCallStatusCodes.PaymasterBalanceChanged });
                    }
                });
            });
        });
