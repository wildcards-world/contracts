...
contract('VersionRegistry', ([account]) => {
    ...
    before('create registry', async () => {
        ...
        jsRegistry = new VersionRegistry_1.VersionRegistry(web3.currentProvider, registryContract.address, { from: account });
    ...
    context('contract param validations', () => {
        it('should fail to add without id', async () => {
            await test_helpers_1.expectRevert(registryContract.addVersion(VersionRegistry_1.string32(''), VersionRegistry_1.string32(''), 'value'), 'missing id');
        });
        it('should fail to add without version', async () => {
            await test_helpers_1.expectRevert(registryContract.addVersion(VersionRegistry_1.string32('id'), VersionRegistry_1.string32(''), 'value'), 'missing version');
        });
    });
    ...
    context('with more versions', () => {
        before(async () => {
            await TestUtils_1.increaseTime(100);
        ...
        context('#getAllVersions', () => {
            ...
            it('should ignore repeated added version (can\'t modify history: only adding to it)', async () => {
                ...
                await registryContract.addVersion(VersionRegistry_1.string32('id'), VersionRegistry_1.string32('ver2'), 'new-value2');
                const versions = await jsRegistry.getAllVersions('id');
                assert.equal(versions.length, 3);
                assert.deepInclude(versions[0], { version: 'ver3', value: 'value3', canceled: false });
                assert.deepInclude(versions[1], { version: 'ver2', value: 'value2', canceled: false });
        ...
        describe('with canceled version', () => {
            before(async () => {
                await registryContract.cancelVersion(VersionRegistry_1.string32('id'), VersionRegistry_1.string32('ver2'), 'reason');
                // at this point:
                // ver1 - 300 sec old
...
//# sourceMappingURL=VersionRegistry.test.js.map